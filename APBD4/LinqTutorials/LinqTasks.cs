...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            var obj = new { Ename = "Brak wartości", Job = (string)null, HireDate = (DateTime?)null };

            var l = new List<object>();
            l.Add(obj);

            IEnumerable<object> result = Emps.Select(e => new { e.Ename, e.Job, e.HireDate }).Union(l);
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.Join(Depts, e => e.Deptno, d => d.Deptno, (e, d) => new { e.Ename, e.Job, d.Dname }).GroupBy(e => e.Dname).Select(e => new Anonymous(e.Key, e.Count())).Where(e => e.NumofEmployees > 1).ToList();
        ...
        {
            int result = arr.GroupBy(n => n).Where(g => g.Count() % 2 == 1).Select(g => g.Key).Single();
            return result;
        }
        ...
        {
            IEnumerable<Dept> result = from b in Depts
                                       join u in Emps on b.Deptno equals u.Deptno into g
    ...
    internal class Anonymous
    {
        ...
        public int NumofEmployees { get; }

        public Anonymous(string name, int numofEmployees)
        {
            Name = name;
        ...
        {
            return HashCode.Combine(Name, NumofEmployees);
        }
        public override string ToString()
        {
            return "{name: " + "\"" + Name + "\", numOfEmployees: " + NumofEmployees + "}";
        }
    }
}
